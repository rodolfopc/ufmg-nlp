...
/**
 * An exhaustive generalized CKY PCFG parser. Fairly carefully optimized to be
 * fast.
 *
 * @author Dan Klein
...
public class ExhaustivePCFGParser implements Scorer, KBestViterbiParser {
	...
	protected int[] words; // words of sentence being parsed as word Numberer
							// ints
	...
	public double scoreBinarizedTree(Tree tree, int start) {
		...
		if (tree.numChildren() == 1) {
			...
			// }
			// return SloppyMath.max(ug.scoreRule(ur), -10000.0) +
		...
		double score = bg.scoreRule(br) + scoreBinarizedTree(tree.children()[0], start)
				+ scoreBinarizedTree(tree.children()[1], start + tree.children()[0].yield().size());
		tree.setScore(score);
		return score;
	}
	...
	protected int[][] narrowRExtent = null; // the leftmost right extent of
											// state s starting at position i
	...
	/**
	 * Just for Parser interface. Remove it when we change Parser interface to
	 * use List<HasWord>
	 *
	 * @param sentence
	...
	public boolean parse(List<? extends HasWord> sentence) { // ExhaustivePCFGParser
		...
		// System.out.println("is it a taggedword?" + (sentence.get(0)
		// instanceof TaggedWord)); //debugging
		...
		if (Test.verbose) {
			// System.out.println(numStates + " states, " + goal + " is the goal
			// state.");
		...
		for (int i = 0; i < length; i++) {
			...
			if (o instanceof HasOffset) {
				offsets[i] = new IntPair(((HasOffset) o).beginPosition(), ((HasOffset) o).endPosition());
			}
		...
		if (Test.doRecovery && !succeeded && !floodTags) {
			...
			// ms: disabled message. this is annoying and it doesn't really
			...
			for (int start = 0; start < length; start++) {
				for (int end = start + 1; end <= length; end++) {
					for (int s = 0; s < numStates; s++) {
						...
						if (oScore[start][end][s] != Float.NEGATIVE_INFINITY
								&& iScore[start][end][s] != Float.NEGATIVE_INFINITY) {
							bNZ++;
						}
					}
				}
	...
	public boolean parse(HTKLatticeReader lr) {
		...
		// (1) The initializeChart() method present in rev. 19820 did not
		// properly initialize
		// lattices (or sub-lattices) like this (where A,B,C are nodes, and NN
		// is the POS tag arc label):
		//
		// --NN--> B --NN--
		// / \
		// A ------NN-------> C
		...
		//
		// To re-implement support for HTKLatticeReader it is necessary to
		// create an interface
		// for the two different lattice implementations and then modify
		// initializeChart() and
		// extractBestParse() as appropriate. Another solution would be to
		// duplicate these two
		// methods and make the necessary changes for HTKLatticeReader. In both
		// cases, the
		// acoustic model score provided by the HTK lattices should be included
		// in the weighting.
		//
		// Note that I never actually tested HTKLatticeReader, so I am uncertain
		// if this facility
	...
	public boolean parse(Lattice lr) { // ExhaustivePCFGParser
		...
		length = lr.getNumNodes() - 1; // Subtract 1 since
										// considerCreatingArrays will add the
										// final interstice
		if (length > arraySize) {
			considerCreatingArrays(length);
		}
		...
		// System.err.println(" " + numStates + " states, " + goal + " is the
		// goal state.");
		// }
	...
	private void doOutsideScores() {
		for (int diff = length; diff >= 1; diff--) {
			for (int start = 0; start + diff <= length; start++) {
				...
				/*
				 * for (int s = 0; s < numStates; s++) { float oS =
				 * oScore[start][end][s]; //if (iScore[start][end][s] ==
				 * Float.NEGATIVE_INFINITY || // oS == Float.NEGATIVE_INFINITY)
				 * if (oS == Float.NEGATIVE_INFINITY) continue; BinaryRule[]
				 * rules = bg.splitRulesWithParent(s); for (int r=0;
				 * r<rules.length; r++) { BinaryRule br = rules[r]; int min1 =
				 * narrowRExtent[start][br.leftChild]; if (end < min1) continue;
				 * int max1 = narrowLExtent[end][br.rightChild]; if (max1 <
				 * min1) continue; int min2 = wideLExtent[end][br.rightChild];
				 * int min = (min1 > min2 ? min1 : min2); if (max1 < min)
				 * continue; int max2 = wideRExtent[start][br.leftChild]; int
				 * max = (max1 < max2 ? max1 : max2); if (max < min) continue;
				 * float pS = (float) br.score; for (int split = min; split <=
				 * max; split++) { float lS =
				 * iScore[start][split][br.leftChild]; if (lS ==
				 * Float.NEGATIVE_INFINITY) continue; float rS =
				 * iScore[split][end][br.rightChild]; if (rS ==
				 * Float.NEGATIVE_INFINITY) continue; float totL = pS+rS+oS; if
				 * (totL > oScore[start][split][br.leftChild]) {
				 * oScore[start][split][br.leftChild] = totL; } float totR =
				 * pS+lS+oS; if (totR > oScore[split][end][br.rightChild]) {
				 * oScore[split][end][br.rightChild] = totR; } } } }
				 */
			}
		}
		...
		for (int diff = 2; diff <= length; diff++) {
			...
			for (int start = 0; start < (diff == length ? 1 : length - diff); start++) {
				...
				for (int leftState = 0; leftState < numStates; leftState++) {
					...
					// if (spillGuts) System.out.println("Found " +
					// leftRules.length + " left rules for state " +
					// stateNumberer.object(leftState));
					for (BinaryRule r : leftRules) {
						// if (spillGuts) System.out.println("Considering rule
						// for " + start + " to " + end + ": " + leftRules[i]);
						...
						if (min > narrowL) { // can this right constituent
						...
						if (min > max) { // can this left constituent stretch
											// far enough to reach the right
											// constituent?
							continue;
						}
						float pS = r.score;
						...
						// System.out.println("Min "+min+" max "+max+" start
						// "+start+" end "+end);

						if (!Test.lengthNormalization) {
							// find the split that can use this rule to make the
							// max score
							for (int split = min; split <= max; split++) {
							...
							// find split that uses this rule to make the max
							...
							for (int split = min; split <= max; split++) {
								...
								float tot = pS + lS + rS;
								int newWordsInSpan = wordsInSpan[start][split][leftState]
										+ wordsInSpan[split][end][r.rightChild];
						...
						if (foundBetter) { // this way of making "parentState"
							...
							// if (spillGuts) System.out.println("Could build "
							// + stateNumberer.object(parentState) + " from " +
							// start + " to " + end);
				...
				for (int rightState = 0; rightState < numStates; rightState++) {
					...
					// if (spillGuts) System.out.println("Found " +
					// rightRules.length + " right rules for state " +
					// stateNumberer.object(rightState));
					for (BinaryRule r : rightRules) {
						// if (spillGuts) System.out.println("Considering rule
						// for " + start + " to " + end + ": " + rightRules[i]);
						...
						// System.out.println("Start "+start+" end "+end+" min
						// "+min+" max "+max);
						if (!Test.lengthNormalization) {
							// find the split that can use this rule to make the
							// max score
							...
							for (int split = min; split <= max; split++) {
								...
								float tot = pS + lS + rS;
								int newWordsInSpan = wordsInSpan[start][split][r.leftChild]
										+ wordsInSpan[split][end][rightState];
								float normTot = tot / newWordsInSpan;
								if (normTot > bestNormIScore) {
									bestIScore = tot;
									bestNormIScore = normTot;
				...
				// do unary rules -- one could promote this loop and put start
				// inside
				for (int state = 0; state < numStates; state++) {
	...
	private void initializeChart(Lattice lr) {
		for (LatticeEdge edge : lr) {
			...
			for (int state = 0; state < numStates; state++) {
				if (isTag[state]) {
					IntTaggedWord itw = new IntTaggedWord(wordNumberer.number(word),
							Numberer.translate(stateSpace, "tags", state));
			...
			// Give scores to all tags if the parse fails (more flexible
			// tagging)
	...
	private void initializeChart(List sentence) {
		...
		for (int start = 0; start + 1 <= length; start++) {
			if (Test.maxSpanForTags > 1) { // only relevant for parsing single
											// words as multiple input tokens.
				// note we don't look for "words" including the end symbol!
				for (int end = start + 1; end < length - 1 && end - start <= Test.maxSpanForTags
						|| start + 1 == end; end++) {
					...
					for (int state = 0; state < numStates; state++) {
						...
						if (iS == Float.NEGATIVE_INFINITY && isTag[state]) {
							IntTaggedWord itw = new IntTaggedWord(wordNumberer.number(word.toString()),
									Numberer.translate(stateSpace, "tags", state));
			...
			} else { // "normal" chart initialization of the [start,start+1]
						// cell
				...
				if (!floodTags || word == boundary) {
					...
					// which may itself be tagging flexibly or using a strict
					// lexicon.
					if (dumpTagging) {
						EncodingPrintWriter.err.println("Normal tagging "
								+ Numberer.getGlobalNumberer("words").object(word) + " [" + word + "]", "UTF-8");
					}
					for (Iterator<IntTaggedWord> taggingI = lex.ruleIteratorByWord(word, start); taggingI.hasNext();) {
						...
						if (trueTagStr != null) { // if word was supplied with a
													// POS tag, skip all
													// taggings not
													// basicCategory()
							...
							if (!Test.forceTagBeginnings && !tlp.basicCategory(tagging.tagString()).equals(trueTagStr)
								...
								if (dumpTagging) {
									EncodingPrintWriter.err.println(
											"  Skipping " + tagging + " as it doesn't match trueTagStr: " + trueTagStr,
											"UTF-8");
								}
								continue;
							}
						...
						// try {
						float lexScore = lex.score(tagging, start); // score the
																	// cell
																	// according
																	// to
																	// P(word|tag)
						...
						// Numberer.getGlobalNumberer("tags").object(tagging.tag));
						// }
						int tag = tagging.tag;
						tags[start][tag] = true;
						if (dumpTagging) {
							EncodingPrintWriter.err.println("Word pos " + start + " tagging " + tagging + " score "
									+ iScore[start][start + 1][state] + " [state " + Numberer.object("states", state)
									+ " = " + state + "]", "UTF-8");
						}
				...
				if (!assignedSomeTag) {
					// If you got here, either you were using forceTags (gold
					// tags)
					// and the gold tag was not seen with that word in the
					// training data
					...
					if (dumpTagging) {
						EncodingPrintWriter.err.println(
								"Forced FlexiTagging " + Numberer.getGlobalNumberer("words").object(word), "UTF-8");
					}
					for (int state = 0; state < numStates; state++) {
						if (isTag[state] && iScore[start][end][state] == Float.NEGATIVE_INFINITY) {
							...
							float lexScore = lex.score(
									new IntTaggedWord(word, Numberer.translate(stateSpace, "tags", state)), start);
							if (lexScore > Float.NEGATIVE_INFINITY) {
								iScore[start][end][state] = lexScore;
								narrowRExtent[start][state] = end;
								narrowLExtent[end][state] = start;
							...
							if (dumpTagging) {
								EncodingPrintWriter.err
										.println(
												"Word pos " + start + " tagging "
														+ new IntTaggedWord(word,
																Numberer.translate(stateSpace, "tags", state))
														+ " score " + iScore[start][start + 1][state] + " [state "
														+ Numberer.object("states", state) + " = " + state + "]",
												"UTF-8");
							}
						}
					}
				...
				if (floodTags && !Test.noRecoveryTagging && !(word == boundary)) {
					// if parse failed because of tag coverage, we put in all
					// tags with
					// a score of -1000, by fiat. You get here from the
					// invocation of
					// parse(ls) inside parse(ls) *after* floodTags has been
					// turned on.
					// Search above for "floodTags = true".
					if (dumpTagging) {
						EncodingPrintWriter.err.println(
								"Flooding tags for " + Numberer.getGlobalNumberer("words").object(word), "UTF-8");
					}
					for (int state = 0; state < numStates; state++) {
						if (isTag[state] && iScore[start][end][state] == Float.NEGATIVE_INFINITY) {
	...
	protected Tree extractBestParse(int goal, int start, int end) {
		...
		// System.err.println("Searching for "+goalStr+" from "+start+" to
		// "+end+" scored "+bestScore +
			...
			if (Test.maxSpanForTags > 1) {
				...
				if (sentence != null) {
					...
					for (LatticeEdge edge : latticeEdges) {
						IntTaggedWord itw = new IntTaggedWord(wordNumberer.number(edge.word),
								Numberer.translate(stateSpace, "tags", goal));
				...
				if (tagScore > Float.NEGATIVE_INFINITY || floodTags) {
					...
					if (offsets[start] != null) {
						leafLabel = new StringLabel(wordStr, offsets[start].getSource(), offsets[start].getTarget());
					} else {
		...
		// we can exploit the sparser space and reconstruct the full tree as we
		// go.
		// for (Iterator<UnaryRule> unaryI =
		// ug.closedRuleIteratorByParent(goal); unaryI.hasNext(); ) {
		for (Iterator<UnaryRule> unaryI = ug.ruleIteratorByParent(goal); unaryI.hasNext();) {
			UnaryRule ur = unaryI.next();
			// System.err.println(" Trying " + ur + " dtr score: " +
			// iScore[start][end][ur.child]);
			...
			if (ur.child != ur.parent && matches) {
				...
				Tree result = tf.newTreeNode(new StringLabel(goalStr), Collections.singletonList(childTree));
	...
	/*
	 * ----------------------- // No longer needed: extracBestParse restores
	 * unaries as it goes protected void restoreUnaries(Tree t) {
	 * //System.out.println("In restoreUnaries..."); for (Tree node : t) {
	 * System.err.println("Doing node: "+node.label()); if (node.isLeaf() ||
	 * node.isPreTerminal() || node.numChildren() != 1) { //System.out.println(
	 * "Skipping node: "+node.label()); continue; } //System.out.println(
	 * "Not skipping node: "+node.label()); Tree parent = node; Tree child =
	 * node.children()[0]; List path =
	 * ug.getBestPath(stateNumberer.number(parent.label().value()),
	 * stateNumberer.number(child.label().value())); System.err.println(
	 * "Got path: "+path); int pos = 1; while (pos < path.size() - 1) { int
	 * interState = ((Integer) path.get(pos)).intValue(); Tree intermediate =
	 * tf.newTreeNode(new StringLabel((String)
	 * stateNumberer.object(interState)), parent.getChildrenAsList());
	 * parent.setChildren(Collections.singletonList(intermediate)); pos++; }
	 * //System.out.println("Done with node: "+node.label()); } }
	 * ----------------------
	 */

	/**
	 * Return all best parses (except no ties allowed on POS tags?). Even though
	 * we parse with the unary-closed grammar, since all the intermediate states
	 * in a chain must have been built, we can reconstruct the unary chain as we
	 * go using the non-unary-closed grammar.
	 */
	protected List<Tree> extractBestParses(int goal, int start, int end) {
		...
		double bestScore = iScore[start][end][goal];
		String goalStr = (String) stateNumberer.object(goal);
		// System.out.println("Searching for "+goalStr+" from "+start+" to
		// "+end+" scored "+bestScore);
		// check tags
	...
	/**
	 * Get k good parses for the sentence. It is expected that the parses
	 * returned approximate the k best parses, but without any guarantee that
	 * the exact list of k best parses has been produced.
	 *
	 * @param k
	 *            The number of good parses to return
	 * @return A list of k good parses for the sentence, with each accompanied
	 *         by its score
	 */
	@Override
	...
	/**
	 * Get k parse samples for the sentence. It is expected that the parses are
	 * sampled based on their relative probability.
	 *
	 * @param k
	 *            The number of sampled parses to return
	 * @return A list of k parse samples for the sentence, with each accompanied
	 *         by its score
	 */
	@Override
	 ...
	 * @return The exact k best parses for the sentence, with each accompanied
	 *         by its score (typically a negative log probability).
	 */
	@Override
	...
	private Tree getTree(Vertex v, int k, int kPrime) {
		...
		if (isTag[v.goal]) {
			...
			if (tagScore > Float.NEGATIVE_INFINITY || floodTags) {
				...
				Tree wordNode = tf.newLeaf(leafLabel);
				return tf.newTreeNode(new StringLabel(goalStr), Collections.singletonList(wordNode));
			} else {
				assert false;
			}
		}
	...
	/**
	 * Get a complete set of the maximally scoring parses for a sentence, rather
	 * than one chosen at random. This set may be of size 1 or larger.
	 *
	 * @return All the equal best parses for a sentence, with each accompanied
	 *         by its score
	 */
	@Override
	...
	protected void createArrays(int length) {
		...
		if (op.doDep && !Test.useFastFactored) {
			// System.out.println("initializing oScore arrays with length " +
