...
public class BaseLexicon implements Lexicon {
  ...
  public BaseLexicon(Options op, Index<String> wordIndex, Index<String> tagIndex) {
    ...
    if (op.lexOptions.uwModel == null) {
      op.lexOptions.uwModel = "edu.stanford.nlp.parser.lexparser.BaseUnknownWordModel";
    }
    ...
    try {
      Class<?> clas = Class.forName(op.lexOptions.uwModel);
      Class<?>[] argClasses = new Class<?>[4];
      argClasses[0] = Options.class;
      argClasses[1] = Lexicon.class;
      argClasses[2] = Index.class;
      argClasses[3] = Index.class;
      ...
      System.err.println("Class not found: " + op.lexOptions.uwModel);
      throw new RuntimeException(e);
    } catch (NoSuchMethodException nsme) {
      System.err.println("Can't construct: " + op.lexOptions.uwModel);
      throw new RuntimeException(nsme);
    } catch (InvocationTargetException ite) {
      System.err.println("Can't construct: " + op.lexOptions.uwModel);
      throw new RuntimeException(ite);
    } catch (InstantiationException e) {
      System.err.println("Couldn't instantiate: " + op.lexOptions.uwModel);
      throw new RuntimeException(e);
    } catch (IllegalAccessException e) {
      System.err.println("Illegal access to " + op.lexOptions.uwModel);
      throw new RuntimeException(e);
    }
  }
    ...
    for (int i = 3; i < args.length; i++) {
      if (lex.isKnown(args[i])) {
        ...
        for (Iterator<IntTaggedWord> it = lex.ruleIteratorByWord(wordIndex.indexOf(args[i], true), i - 3, null); it.hasNext(); ) {
          ...
          System.out.println(StringUtils.pad(iTW, 24) + nf.format(lex.score(iTW, i - 3, wordIndex.get(iTW.word))));
        }
      } else {
